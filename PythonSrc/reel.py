...
class Reel:
    ...
    def move(self, y):
        if y > self.reel[0][1].height:
            for _ in range(1, y//self.card_size[1]):
                self.move(self.card_size[1])
        for curr in self.reel:
            curr[1].move_ip((0, y))
        reel_position_rect = (self.reel[self.reel_position])[1]
        if reel_position_rect.bottomleft[1] > reel_position_rect.height * 2:  # if self.reel_position leaves screen

            self.reel_position = (self.reel_position + 1) % len(self.reel)  # update reel Position
            reel_position_rect = (self.reel[self.reel_position])[1]
            new_position = reel_position_rect.topleft[1] - reel_position_rect.height  # determine where the 2nd reel
